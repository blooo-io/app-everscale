...
UX_STEP_CB(ux_sign_flow_3_step, pbb, send_response(0, false),
           {
               &C_icon_crossmark,
               "Cancel",
               "signature",
           });
UX_STEP_CB(ux_sign_flow_4_step, pbb, send_response(set_result_sign(), true),
           {
               &C_icon_validate_14,
               "Sign",
               "message.",
...
void handleSign(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  if (!context->sign_with_chain_id) {
    ...
    snprintf(context->to_sign_str, sizeof(context->to_sign_str), "%.*H",
             TO_SIGN_LENGTH, context->to_sign);
  } else {
    memcpy(context->to_sign, context->chain_id, CHAIN_ID_LENGTH);
    memcpy(context->to_sign + CHAIN_ID_LENGTH, dataBuffer + offset,
           TO_SIGN_LENGTH);
    snprintf(context->to_sign_str, sizeof(context->to_sign_str), "%.*H",
             CHAIN_ID_LENGTH + TO_SIGN_LENGTH, context->to_sign);
  }

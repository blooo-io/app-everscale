...
void SliceData_init(struct SliceData_t *self, uint8_t *data,
...
void SliceData_fill(struct SliceData_t *self, uint8_t value,
                    uint16_t data_size_bytes) {
...
void SliceData_move_by(struct SliceData_t *self, uint16_t offset) {
  VALIDATE(self && self->data, ERR_SLICE_IS_EMPTY);
  VALIDATE(self->data_window_start + offset <= self->data_window_end,
           ERR_CELL_UNDERFLOW);
  self->data_window_start += offset;
}

uint8_t SliceData_get_bits(const struct SliceData_t *self, uint16_t offset,
                           uint8_t bits) {
  VALIDATE(self && self->data, ERR_SLICE_IS_EMPTY);
...
bool SliceData_equal(const struct SliceData_t *self,
                     const struct SliceData_t *other) {
...
void SliceData_append(struct SliceData_t *self, uint8_t *in, uint16_t bits,
                      bool append_tag) {
  uint8_t bytes = bits / 8;
  ...
  if (offset % 8 == 0 || bytes == 0) {
    ...
    for (uint16_t i = first_data_byte, j = 0; j < bytes; ++i, ++j) {
      VALIDATE(i == (j + first_data_byte) && i < self->data_size_bytes,
               ERR_INVALID_DATA);

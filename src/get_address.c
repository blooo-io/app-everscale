...
static uint8_t set_result_get_address() {
  ...
  memmove(G_io_apdu_buffer + tx, data_context.addr_context.address,
          ADDRESS_LENGTH);
  tx += ADDRESS_LENGTH;
  reset_app_context();
  return tx;
...
UX_STEP_NOCB(ux_display_address_flow_2_step, bnnn_paging,
             {
                 .title = "Address",
                 .text = data_context.addr_context.address_str,
             });
UX_STEP_CB(ux_display_address_flow_3_step, pb, send_response(0, false),
           {
               &C_icon_crossmark,
               "Reject",
           });
UX_STEP_CB(ux_display_address_flow_4_step, pb,
...
void handleGetAddress(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
                      uint16_t dataLength, volatile unsigned int *flags,
                      volatile unsigned int *tx) {
  VALIDATE(p2 == 0 && dataLength == (sizeof(uint32_t) + sizeof(uint8_t)),
           ERR_INVALID_REQUEST);
  ...
  if (p1 == P1_CONFIRM) {
    ...
    snprintf(context->address_str, sizeof(context->address_str), "%.*H",
             sizeof(context->address), context->address);

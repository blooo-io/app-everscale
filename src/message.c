...
void deserialize_array(uint8_t *in, uint8_t in_size, uint16_t offset,
  ...
  for (uint16_t i = first_data_byte, j = 0; j < out_size; ++i, ++j) {
    VALIDATE(i == (j + first_data_byte) && (i + 1) <= in_size,
             ERR_INVALID_DATA);
...
void deserialize_address(struct SliceData_t *slice, int8_t *wc,
                         uint8_t *address) {
...
void deserialize_value(struct SliceData_t *slice, uint8_t *value,
...
void set_dst_address(uint8_t wc, const uint8_t *address) {
  ...
  snprintf(address_str,
           sizeof(data_context.sign_tr_context.address_str) - wc_len, "%.*H",
...
void set_amount(const uint8_t *amount, uint8_t amount_length, uint8_t flags,
  ...
  switch (flags) {
  case NORMAL_FLAG: {
    uint8_t text_size = convert_hex_amount_to_displayable(
        amount, decimals, amount_length, amount_str);
...
void set_transaction_id(const uint8_t *transaction_id) {
  ...
  memset(transaction_id_str, 0,
         sizeof(data_context.sign_tr_context.transaction_id_str));

  snprintf(transaction_id_str,
           sizeof(data_context.sign_tr_context.transaction_id_str), "%.*H",
           TRANSACTION_ID_LENGTH, transaction_id);
}

void deserialize_int_message_header(struct SliceData_t *slice, uint8_t flags,
...
int deserialize_multisig_params(struct SliceData_t *slice, uint32_t function_id,
                                uint8_t wc, uint8_t *address,
                                SignTransactionContext_t *ctx) {
  int sign_transaction_flow = SIGN_TRANSACTION_FLOW_ERROR;
...
void prepare_payload_hash(BocContext_t *bc) {
  ...
  if (!data_context.sign_tr_context.sign_with_chain_id) {
    memcpy(data_context.sign_tr_context.to_sign,
           &bc->hashes[ROOT_CELL_INDEX * HASH_SIZE], TO_SIGN_LENGTH);
  } else {
    memcpy(data_context.sign_tr_context.to_sign,
           data_context.sign_tr_context.chain_id, CHAIN_ID_LENGTH);
...
void prepend_address_to_cell(uint8_t *cell_buffer, uint16_t cell_buffer_size,
                             struct Cell_t *cell, uint8_t wc,
                             uint8_t *address) {
...
int prepare_to_sign(struct ByteStream_t *src, uint8_t wc, uint8_t *address,
  ...
  case SURF_WALLET: {
    ...
    sign_transaction_flow = deserialize_multisig_params(
        &gift_slice, function_id, wc, address, &dc->sign_tr_context);
  ...
  case MULTISIG_2_1: {
    ...
    prepend_address_to_cell(cell_buffer, sizeof(cell_buffer), root_cell, wc,
                            prepend_address);

...
static uint8_t set_result_get_public_key() {
  ...
  memmove(G_io_apdu_buffer + tx, data_context.pk_context.public_key,
          PUBLIC_KEY_LENGTH);
...
UX_STEP_NOCB(ux_display_public_flow_1_step, bnnn_paging,
             {
                 .title = "Public key",
                 .text = data_context.pk_context.public_key_str,
             });
UX_STEP_CB(ux_display_public_flow_2_step, pb, send_response(0, false),
           {
               &C_icon_crossmark,
               "Reject",
           });
UX_STEP_CB(ux_display_public_flow_3_step, pb,
...
void handleGetPublicKey(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  if (p1 == P1_CONFIRM) {
    snprintf(context->public_key_str, sizeof(context->public_key_str), "%.*H",
             sizeof(context->public_key), context->public_key);

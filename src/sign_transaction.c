  ...
  BEGIN_TRY {
    TRY {
      ...
      if (!context->sign_with_chain_id) {
        cx_eddsa_sign(&privateKey, CX_LAST, CX_SHA512, context->to_sign,
                      TO_SIGN_LENGTH, NULL, 0, context->signature,
                      SIGNATURE_LENGTH, NULL);
      } else {
        cx_eddsa_sign(&privateKey, CX_LAST, CX_SHA512, context->to_sign,
                      CHAIN_ID_LENGTH + TO_SIGN_LENGTH, NULL, 0,
                      context->signature, SIGNATURE_LENGTH, NULL);
      }
    }
    FINALLY { memset(&privateKey, 0, sizeof(privateKey)); }
  }
  END_TRY;
...
UX_STEP_NOCB(ux_sign_transaction_intro, pnn,
             {
                 &C_icon_eye,
                 "Review",
                 "transaction",
             });
UX_STEP_NOCB(ux_sign_transaction_burn, bnnn_paging,
             {.title = "Action", .text = "Burn"});
UX_STEP_NOCB(ux_sign_transaction_deploy, bnnn_paging,
             {.title = "Action", .text = "Deploy"});
UX_STEP_NOCB(ux_sign_transaction_confirm, bnnn_paging,
             {.title = "Action", .text = "Confirm"});
UX_STEP_NOCB(ux_sign_transaction_transfer, bnnn_paging,
             {.title = "Action", .text = "Transfer"});
UX_STEP_NOCB(ux_sign_transaction_amount, bnnn_paging,
             {
                 .title = "Amount",
                 .text = data_context.sign_tr_context.amount_str,
             });
UX_STEP_NOCB(ux_sign_transaction_address, bnnn_paging,
             {
                 .title = "Address",
                 .text = data_context.sign_tr_context.address_str,
             });
UX_STEP_NOCB(ux_sign_transaction_transaction_id, bnnn_paging,
             {
                 .title = "Transaction id",
                 .text = data_context.sign_tr_context.transaction_id_str,
             });
UX_STEP_CB(ux_sign_transaction_accept, pbb,
...
UX_STEP_CB(ux_sign_transaction_reject, pb, send_response(0, false),
           {
               &C_icon_crossmark,
               "Reject",
           });

UX_FLOW(ux_sign_transaction_burn_flow, &ux_sign_transaction_intro,
        &ux_sign_transaction_burn, &ux_sign_transaction_amount,
        &ux_sign_transaction_accept, &ux_sign_transaction_reject);

UX_FLOW(ux_sign_transaction_deploy_flow, &ux_sign_transaction_intro,
        &ux_sign_transaction_deploy, &ux_sign_transaction_address,
        &ux_sign_transaction_accept, &ux_sign_transaction_reject);

UX_FLOW(ux_sign_transaction_confirm_flow, &ux_sign_transaction_intro,
        &ux_sign_transaction_confirm, &ux_sign_transaction_transaction_id,
        &ux_sign_transaction_accept, &ux_sign_transaction_reject);

UX_FLOW(ux_sign_transaction_transfer_flow, &ux_sign_transaction_intro,
        &ux_sign_transaction_transfer, &ux_sign_transaction_amount,
        &ux_sign_transaction_address, &ux_sign_transaction_accept,
        &ux_sign_transaction_reject);

void handleSignTransaction(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,

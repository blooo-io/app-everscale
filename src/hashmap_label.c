...
uint8_t get_label_same(uint8_t max, struct SliceData_t *slice,
                       struct SliceData_t *label) {
...
uint8_t get_label(uint8_t max, struct SliceData_t *slice,
                  struct SliceData_t *label) {
...
void put_to_node(uint8_t cell_index, uint16_t bit_len,
                 struct SliceData_t *key) {
  VALIDATE(cell_index != 0 && cell_index <= MAX_CONTRACT_CELLS_COUNT,
           ERR_INVALID_DATA);
  ...
  VALIDATE(next_cell != 0 && next_cell <= MAX_CONTRACT_CELLS_COUNT,
           ERR_INVALID_DATA);
  bit_len -= 1;

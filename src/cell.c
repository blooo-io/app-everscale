...
uint16_t deserialize_cell(struct Cell_t *cell, const uint8_t cell_index,
  ...
  VALIDATE(!exotic || pruned,
           ERR_INVALID_DATA); // only ordinary or pruned cells are valid
  ...
  VALIDATE(!pruned || pruned && (data_size == PRUNED_BRANCH_DATA_SIZE),
           ERR_INVALID_DATA);
...
void calc_cell_hash(Cell_t *cell, const uint8_t cell_index) {
  ...
  uint8_t
      hash_buffer[266]; // d1(1) + d2(1) + data(128) + 4 * (depth(2) + hash(32))
  ...
  if (bc->public_key_cell_index && cell_index == bc->public_key_cell_index) {
    memcpy(hash_buffer + hash_buffer_offset, bc->public_key_cell_data,
           data_size);
  } else {
  ...
  int result = cx_hash_sha256(hash_buffer, hash_buffer_offset,
                              bc->hashes + cell_index * HASH_SIZE, HASH_SIZE);
